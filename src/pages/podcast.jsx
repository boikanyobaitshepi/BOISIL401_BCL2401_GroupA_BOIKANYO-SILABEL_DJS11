          ...
          <div className="col p-1">
            <div className="btn-group me">
              ...
              <div className="dropdown-menu">
                <button className="dropdown-item" onClick={() => handleFilterChange('A-Z')}>A-Z</button>
                <button className="dropdown-item" onClick={() => handleFilterChange('Z-A')}>Z-A</button>
                <button className="dropdown-item" onClick={() => handleFilterChange('ascending')}>Oldest</button>
                <button className="dropdown-item" onClick={() => handleFilterChange('descending')}>Newest</button>
              </div>
            </div>
            <div className="ms-2">
              {Object.keys(genreMapping).map((key) => (
                <Link key={key} to={`?genres=${key}`} className={`btn btn-outline-secondary rounded-pill me-2 ${genresFilter === parseInt(key) ? 'active' : ''}`}>
                  {genreMapping[key]}
                </Link>
              ))}
                ...
                <div className="card">
                    ...
                    <ul className="list-group list-group-flush bg-dark">
                      ...
                      <li className="list-group-item">
                        Genres: {show.genres.map((genreId) => genreMapping[genreId]).join(', ')} 
                        <img className='ms-3' src="https://www.flaticon.com/free-icons/star" alt="heart" width="18px" height="18px" />
                      </li>
                    </ul>
                  </div>
                </Link>
